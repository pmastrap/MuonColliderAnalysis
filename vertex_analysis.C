...
void vertex_analysis(){
	...
	/// PLOT 3b //////////////////////////////
	TH1F* SV_to_PV_distance_j=new TH1F("SV_to_PV_distance_j","SV_to_PV_distance_j",200,0.,5);
	...
	TH1F* SV_to_PV_distance_sig_j=new TH1F("SV_to_PV_distance_sig_j","SV_to_PV_distance_sig_j",150,0.,50);
	...
	TH2F *dum_pt=new TH2F("dum_pt","dum_pt",20,0,50,30,-2.6,2.6);
	...
	/*if(nj_b!=nj){
		cout<<"Not all jets contains B-hadrons"<<endl;
		for(i=0;i<nj;i++){
			jet_mo.SetXYZ(jmox[i],jmoy[i],jmoz[i]);
			cout<<i<<": pT = "<<jet_mo.Pt()<<" eta: "<<jet_mo.Eta()<<endl;
			}
		}*/
	for(i=0;i<nj_light;i++){
		jet_mo.SetXYZ(jmox[j_light[i]],jmoy[j_light[i]],jmoz[j_light[i]]);
		dum_pt->Fill(jet_mo.Pt(),jet_mo.Eta());
		}
  		...
  		for(i=0;i<nrec;i++){
  			...
  			if(rcvts[i]!=-1){
  				...
  				for(n=0;n<r2mnrel;n++){
  					...
  					//tot tracks from bottom
  					else if(selection_bb(mcpdg[mcpa0[r2mt[n]]])==0){
  						tot_trk_b++;
  						}
  					//tot tracks from charm
  					else if(selection_cc(mcpdg[mcpa0[r2mt[n]]])==0){
  						tot_trk_c++;
  						}
  					//all other tracks
  					else{
  					...
  					//trk in SV from bottom
  					else if(selection_bb(mcpdg[mcpa0[r2mt[n]]])==0&&id[rcvts[i]]==104){	
  						trk_SV_b++;
  						}
  					//trk in SV from charm
  					else if(selection_cc(mcpdg[mcpa0[r2mt[n]]])==0&&id[rcvts[i]]==104){	
  						trk_SV_c++;
  						}
  					//all other tracks
  					else if(id[rcvts[i]]==104){
  						trk_SV_other++;
  						}
  					////////////////////////////////////////////////////////////////////////////////////////
  					////////////////////////////////////////////////////////////////////////////////////////
  				if(r2mw[n]>0.9){
  					...
  					if((selection_cc(mcpdg[mcpa0[r2mt[n]]])==0||selection_bb(mcpdg[mcpa0[r2mt[n]]])==0)&&id[rcvts[i]]==104){	
  		...
  		for(k=0;k<nvt;k++){
  			if(id[k]==103){
  				...
  				if(same_parent==1){
  					...
  					if(selection_bb(mcpdg[prev_parent])==0){
  					...
  					else if(selection_cc(mcpdg[prev_parent])==0){
