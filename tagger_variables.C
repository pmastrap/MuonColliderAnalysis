...
void tagger_variables(){
	...
	double decay_length_trk2;
   	...
   	//for analysis
   	tree->Branch("j_px", &j_px,"j_px/D");
   	tree->Branch("j_py", &j_py,"j_py/D");
   	tree->Branch("j_pz", &j_pz,"j_pz/D");
   	tree->Branch("j_E", &j_E,"j_E/D");
   	//flavour flag
   	//1 -> flavour = choice 
   	//0 -> flavour != choice
   	tree->Branch("j_flavour",&j_flavour,"j_flavour/I");
	...
	TChain* fChain = new TChain("fChain");
   	...
   	//fChain->Add("/home/paola/Scrivania/xml_aggiornati/Bkg/ntuple_udsg_10000_fix.root/MyLCTuple");
	...
	int jet_tot=0,jet_sel=0,trk_tot=0,trk_sel=0,vtx_tot=0,vtx_sel=0,lep_tot=0,lep_sel=0;
	...
	for(ientry=0; ientry<fChain->GetEntries() ; ++ientry){
  		...
  		for(k=0;k<100;k++){
  			...
  			sprintf(Jet[k].flavour_tag,"null");
	...
	cout<<"--------------------------------------------"<<endl;*/
		...
		//fill Track Structure
  		for(i=0;i<nrec;i++){
  			if(rcftr[i]!=-1){
  				...
  				if(Track[i].selected==true){trk_sel++;}
  				if(abs(rctyp[i])==13 || abs(rctyp[i])==11){
  					lep_tot++;
  					if(Track[i].selected==true && lep_selection(Track[i])==true){
  						lep_sel++;
  						}
  					}
  				trk_tot++;
  		...
  		for (k=0;k<nvt; k++){
  			...
  			if(Vertex[k].selected==true){vtx_sel++;}
  			if(Vertex[k].id==104){vtx_tot++;}
  			}
  		...
  		for(n=0;n<nj;n++){
  			...
  			for(k=0;k<Jet[n].num_trk;k++){
  				if(abs(rctyp[Jet[n].trk_indices[k]])==13 || abs(rctyp[Jet[n].trk_indices[k]])==11){
  					if(Track[Jet[n].trk_indices[k]].selected==true && lep_selection(Track[Jet[n].trk_indices[k]])==true){
  		...
  		for(k=0;k<nj;k++){
  			jet_tot++;
  			//&& strcmp(Jet[k].flavour_tag,choice)==0
  			if(Jet[k].selected==true&& strcmp(Jet[k].flavour_tag,choice)==0){
  				...
  				j_pz=Jet[k].j.Pz();
  				j_E=Jet[k].j.E();
  				if(strcmp(Jet[k].flavour_tag,choice)==0) j_flavour=1;
  				else j_flavour=0;
  				event_id=ientry;
  				...
  				while(ordered_indeces[n]!=-1 && Track[Jet[k].trk_indices[ordered_indeces[n]]].selected==true &&
  				 inv_mass<inv_mass_thr){
  	...
  	tree->Print();
  	cout<<"tot jet: "<<jet_tot<<endl;
  	cout<<"sel jet: "<<jet_sel<<endl;
  	cout<<"tot trk: "<<trk_tot<<endl;
  	cout<<"sel trk: "<<trk_sel<<endl;
  	cout<<"tot vtx: "<<vtx_tot<<endl;
  	cout<<"sel vtx: "<<vtx_sel<<endl;
  	cout<<"tot lep: "<<lep_tot<<endl;
  	cout<<"sel lep: "<<lep_sel<<endl;
  		
  	//FILL TREES FOR MVA
	TFile *outfile = new TFile(Form("./%s_tree_forMVA_10k.root",choice), "recreate");
      	outfile->cd("");	
      	tree->Write();
      	outfile->Close();
