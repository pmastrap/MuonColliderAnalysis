...
void MVA_c_to_light_jets_discrimination(){

	// Output file
	TFile *fout = new TFile("TMVA_output_10k_3_light.root", "RECREATE");
	...
	//BKG tree
	TFile *f_bkg = new TFile("/home/paola/Scrivania/macro/Trees_tagger/o_tree_forMVA3_10k.root");
	...
	Double_t sigWeight1  = 1.0;
    	...
    	DataLoader *dataloader = new DataLoader("dataset_10k_3_light");
   	...
   	dataloader->AddBackgroundTree(bkgTree,bkgWeight1);
    	...
    	/*dataloader->AddVariable("n_trk_in_Jet", 'I');
    	dataloader->AddVariable("pl_rel_TRK1", 'D');
    	dataloader->AddVariable("pt_rel_TRK1", 'D');
    	dataloader->AddVariable("dR_jet_tot_TRK", 'D');*/
    ...
    if (!gROOT->IsBatch()){
        TMVAGui("TMVA_output_10k_3_light.root");
    	}
