...
void MVA_cjets_discrimination_application(string choice ="signal"){
	...
	myTree->SetBranchAddress("event_id",&event_id);
  	...
  	float  forBDTevaluation3; 
   	...
   	/*reader->AddVariable("vertex_cat", &forBDTevaluation1 );
   	reader->AddVariable("n_SV", &forBDTevaluation2 );
    	reader->AddVariable("n_trk_from_SV", &forBDTevaluation3);
    	reader->AddVariable("fd_sig_xy_SV", &forBDTevaluation4);
    	reader->AddVariable("fd_sig_xyz_SV", &forBDTevaluation5);
    	reader->AddVariable("corrected_mass_SV", &forBDTevaluation6);
    	reader->AddVariable("MassEnergyFraction_SV", &forBDTevaluation7);
    	reader->AddVariable("Boost_SV", &forBDTevaluation8);
    	reader->AddVariable("energy_ratio_SV", &forBDTevaluation9);
    	reader->AddVariable("dR_SV_JET", &forBDTevaluation10);*/
    	reader->AddVariable("SIP_sig_xy_TRK1", &forBDTevaluation11);
    	reader->AddVariable("SIP_sig_xy_TRK2", &forBDTevaluation12);
    	reader->AddVariable("SIP_sig_xyz_TRK1", &forBDTevaluation13);
    	reader->AddVariable("SIP_sig_xyz_TRK2", &forBDTevaluation14);
    	reader->AddVariable("SIP_sig_xy_TRK_above_trh", &forBDTevaluation15);
    	reader->AddVariable("SIP_sig_xyz_TRK_above_trh", &forBDTevaluation16);
    	reader->AddVariable("dR_jet_TRK1", &forBDTevaluation17);
    	reader->AddVariable("lepton_cat", &forBDTevaluation18);
    	reader->AddVariable("pt_lj_LEP1", &forBDTevaluation19);
    	reader->AddVariable("Et_trk_Jet", &forBDTevaluation20);
    	
    	
	reader->TMVA::Reader::BookMVA("BDT", weight_path_L);
  	cout<<"Using weights in "<<weight_path_L<<endl;
  	...
  	for (long int jentry=0; jentry<nentries; jentry++) {
		...
		forBDTevaluation20 = Et_trk_Jet;
    		...
    		if( BDT_decision >= CvsL ){//selected
      		...
      		else if ( BDT_decision < CvsL ){ //rejected
           		rejected++;
           		}
	...
	std::cout << "==> Selected jets " << selected << std::endl;
    	std::cout << "==> Rejected jets " << rejected << std::endl;
	...
	tag_eff->SetLineColor(kBlack);
  	...
  	tag_eff->Draw("ap");
    	...
    	TFile* f=new TFile(Form("/home/paola/Scrivania/InvMass_%s_light.root",choice.c_str()),"RECREATE");
